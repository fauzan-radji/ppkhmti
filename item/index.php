  ...
  <div class="card-body">
    ...
    <table class="table table-stripped" id="dataTable">
      ...
      <tbody class="text-center">
        ...
        $sql = "SELECT items.*, kategori.nama as nama_kategori FROM items JOIN kategori ON items.item_kategori = kategori.id";
        ...
        ?>
          <tr>
            ...
            <td>
              ...
              <a href='item/delete.php?id=<?= $data['id'] ?>' onclick="return confirm('Anda yakin ingin menghapus data ini?')" class='btn btn-sm btn-danger rounded-circle'><i class='fas fa-trash'></i></a>
            </td>
          </tr>
        <?php endwhile; ?>
      </tbody>
...
for ($kategori = array(); $row = $result->fetch_assoc(); $kategori[] = $row);
?>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="item/update.php" method="post" enctype="multipart/form-data">
          ...
          <div class="row mb-3">
            <div class="col">
              <textarea id="ubahDeskripsi" type="text" rows="7" class="form-control" placeholder="Deskripsi Barang" name="desc_barang"></textarea>
            </div>
          </div>
          <div class="row mb-3">
            <div class="col">
...
// Modal Ubah
$content = "<form action='item/update.php' method='post' enctype='multipart/form-data'>
<input id='ubahId' type='hidden' name='id'>
<div class='row mb-3'>
  <div class='col'>
    <input id='ubahNama' type='text' class='form-control' placeholder='Nama Barang' name='nama_barang'>
...
<div class='row mb-3'>
  <div class='col'>
    <select name='kategori_barang' class='form-control'>
      <option value=''>Kategori Barang</option>";
foreach ($kategori as $data) $content .= "<option id='ubahKategori-{$data['id']}' value='{$data['id']}'>{$data['nama']}</option>";
$content .= "
    </select>
  </div>
</div>
...
<div class='card-body'>
  <h4 id='detailNama' class='card-title'>Reza Kecap</h4>
  <h5 id='detailDeskripsi' class='card-subtitle'>Ketua Bidang Pengembangan Organisasi</h5>
  <p id='detailKategori' class='card-text mt-3'>Bahan Makanan</p>
</div>
<div class='card-footer'>
  <p>Rp. <span id='detailHarga'>50000</span></p>
</div>
</div>")
  ->print();
?>
